...
This graph is better in terms of visual appearance and labels and includes much more detail, but the legend is too big and the graph is squished, the colours are too similar and theres just too much going on in the graph to read the data clearly, so its still unclear.
...
alzheimer_plot<- ggplot(data = europe_alzheimers_deaths, #calling the data
  ...
  aes(
    ...
    group = Country,
    colour = Country,
    #adding x and y axis, grouping by country and making each country a different colour, I couldn't find a colour palette that covers enough colours to assign a different clear colour to each coutry better than the default  
)) +
...
alzheimers_heatmap<- europe_alzheimers_deaths %>% #naming the graph
  ...
  labs(title = "Heatmap of Death Rates from Alzeimers and other dementias across Europe 2000-2021",
