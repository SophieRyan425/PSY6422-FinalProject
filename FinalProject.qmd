...
#6 Viewing the new clean data set 
view(europe_alzheimers_df)

#7 Renaming columns 
europe_alzheimers_deaths<- europe_alzheimers_df %>%
...
ggplot(europe_alzheimers_deaths, aes(x = Year, y = DeathRate, colour = Country, group = Country)) + 
  ...
  y = "Number of Deaths",
  colour = "Country") +
  #adding title and labels for axis and assigning a different colour for each country 
 
   theme_minimal()
```

This was just a basic line graph for me to get an idea on how i can work with the data i have and what sort of graph i could create. This one is very simple and i don't like the proportions and it does not look that great so i am going to try and advance this visualisation into something clearer and more precise.

```{r}
alzheimer_plot<- ggplot(
  data = europe_alzheimers_deaths,
  aes(
    x = Year,
    y = DeathRate,
    group = Country,
...
This graph is better in terms of visual appearance and labels and includes much more detail, but the legend is too big and the graph is squished so its still unclear.
